...
function hienThiDanhSachGhe(data) {
    data.forEach((hang) => {
        console.log(`Hàng ${hang.hang}:`);
        hang.danhSachGhe.forEach((ghe) => {
            if (ghe.daDat) {
                console.log(`${ghe.soGhe} - Đã đặt`);
            } else {
                console.log(`${ghe.soGhe} - Chưa đặt - Giá: ${ghe.gia}`);
            }
        });
    });
}

function datChoGhe(data, hang, soGhe) {
    const hangGhe = data.find((item) => item.hang === hang);
    if (hangGhe) {
        const ghe = hangGhe.danhSachGhe.find((item) => item.soGhe === soGhe);
        if (ghe && !ghe.daDat) {
            ghe.daDat = true;
            console.log(`Đặt chỗ thành công: ${ghe.soGhe}`);
        } else {
            console.log(`Ghế ${soGhe} đã được đặt hoặc không tồn tại.`);
        }
    } else {
        console.log(`Hàng ${hang} không tồn tại.`);
    }
}

function huyDatChoGhe(data, hang, soGhe) {
    const hangGhe = data.find((item) => item.hang === hang);
    if (hangGhe) {
        const ghe = hangGhe.danhSachGhe.find((item) => item.soGhe === soGhe);
        if (ghe && ghe.daDat) {
            ghe.daDat = false;
            console.log(`Hủy đặt chỗ thành công: ${ghe.soGhe}`);
        } else {
            console.log(`Ghế ${soGhe} chưa được đặt hoặc không tồn tại.`);
        }
    } else {
        console.log(`Hàng ${hang} không tồn tại.`);
    }
...
function tinhTongGheVaTongTien(data) {
    let tongGhe = 0;
    let tongTien = 0;
    data.forEach((hang) => {
        hang.danhSachGhe.forEach((ghe) => {
    ...
    console.log(`Tổng số ghế đã đặt: ${tongGhe}`);
